...
fn serialize_to_byte_string(queries: Vec<String>) -> Vec<u8> {
    queries
        ...
        .flat_map(|query: &String| {
            ...
            len_bytes
                .into_iter()
                .chain(query.as_bytes().to_vec().iter().copied())
        })
        .collect::<Vec<u8>>()
}

async fn inference(queries: Vec<String>) -> ModelInferResponse {
    let shape: Vec<i64> = vec![queries.len() as i64, 1];
...
pub async fn get_embedding(queries: Vec<String>) -> Array2<f32> {
    ...
    flattend_vectors[0].clone().into_shape((batch_size, V1_EMBEDDING_SIZE)).unwrap()
}
